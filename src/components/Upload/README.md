...
### MarkdownImage Component example

Renders an upload button in the toolbar of a textarea with the option `{ md: true }` assign this value to the key 'toolbar': `toolbar: [[ markdownImage ]]`
...
const { images } = config.storage || {} // your image host location, with trailing slash
...
const MarkdownImageModal = ({ handler }) => {
  ...
  return (
    <Modal toggle={toggle} isOpen={isOpen} className={'admin-dashboard__style'}> // for use in @lefapps/admindashboard
...
const markdownImage = ({ value, cursor }) =>
  new Promise((resolve, reject) => {
    ...
    document
      .querySelector('.admin-board__full .admin-board__body') // for use in @lefapps/admindashboard
      .append(parentNode)
    const handler = image => {
      if (image) {
